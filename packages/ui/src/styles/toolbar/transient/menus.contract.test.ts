...
describe('toolbar-transient-menus contract', () => {
  it('keeps menu triggers, placement animation, and viewport/capture variants on the transient owner', () => {
    ...
    expect(toolbarTransientMenusStylesheet).toMatch(
      /\.sniptale-toolbar \.sniptale-full-page-chevron \{[^}]*flex: 0 0 18px;[^}]*min-width: 18px;[^}]*width: 18px;[^}]*max-width: 18px;[^}]*padding: 0;[^}]*border-inline-start-width: 0;/su
    );
    expect(toolbarTransientMenusStylesheet).toMatch(
      /\.sniptale-toolbar \.sniptale-full-page-chevron svg \{[^}]*width: 12px;[^}]*height: 12px;/su
    );
  ...
  it('treats the full-page split action as one hover and focus surface', () => {
    ...
    const sharedHoverRule = toolbarTransientMenusStylesheet.match(
      /\.sniptale-toolbar\s+\.sniptale-full-page-wrapper:not\(\[data-active='true'\]\):hover\s+\.sniptale-btn:not\(:disabled\)\s*\{[^}]*\}/su
    )?.[0];
    expect(sharedHoverRule).toContain('background: var(--sniptale-hover-bg);');
    expect(toolbarTransientMenusStylesheet).toMatch(
      /\.sniptale-toolbar \.sniptale-full-page-wrapper > \.sniptale-btn\.sniptale-btn\s*\{[^}]*border-color:\s*transparent;[^}]*box-shadow:\s*none;/su
    );
    expect(sharedHoverRule).toContain('border-color: transparent;');
    expect(toolbarTransientMenusStylesheet).toMatch(
      /\.sniptale-toolbar\s+\.sniptale-full-page-wrapper:not\(\[data-active='true'\]\):has\(\s*\.sniptale-btn:not\(:disabled\)\s*\):hover::after/su
    );
