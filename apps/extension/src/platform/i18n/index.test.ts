...
describe('Popup video controls i18n', () => {
  it('uses concise countdown and size-preset management copy in both locales', () => {
    ...
    expect(translate('popup.video.manageSizePresets', 'ru')).toBe(
      'Управление шаблонами размера окна…'
    );
    expect(translate('popup.video.manageSizePresets', 'en')).toBe('Manage window size presets…');
  });
});
