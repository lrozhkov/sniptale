...
function MockProductModal({
  ...
  return isOpen ? (
    <div className={dialogClassName} data-testid="product-modal" {...props}>
      {children}
    </div>
  ) : null;
}
...
describe('BorderPresetEditorContent', () => {
  ...
  it('disables and marks save busy while persistence is pending', async () => {
    await renderComponent(
      <BorderPresetEditorContent isSaving onClose={vi.fn()} state={createState()} />
    );
...
describe('BorderPresetEditorContent edit mode', () => {
  ...
  it('closes on Escape without consuming ordinary editor keys', async () => {
    const onClose = vi.fn();
    await renderComponent(
      <BorderPresetEditorContent onClose={onClose} preset={createPreset()} state={createState()} />
    );
    const modal = container?.querySelector<HTMLElement>('[data-testid="product-modal"]');
    if (!modal) throw new Error('Expected product modal');

    await act(async () => {
      modal.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true, key: 'Enter' }));
    });
  ...
  it('moves focus inside the dialog, traps Tab, and restores the trigger on close', async () => {
    ...
    if (!modal || buttons.length < 2) throw new Error('Expected focusable product modal');
