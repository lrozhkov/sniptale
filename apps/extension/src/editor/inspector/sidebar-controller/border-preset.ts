...
export function createBorderPresetFromShapeSettings(
  ...
  const strokeOpacity = clampPercentUnit(shapeSettings.strokeOpacity, shapeSettings.opacity) / 100;
  const fillOpacity = clampPercentUnit(shapeSettings.fillOpacity, shapeSettings.opacity) / 100;
  const order =
  ...
  return {
    ...
    color:
      multiplyColorAlpha(shapeSettings.strokeColor, strokeOpacity) ?? shapeSettings.strokeColor,
