...
import {
  FrameCalloutBadge as CalloutBadge,
  resolveFrameCalloutBadgeText as resolveCalloutBadgeText,
} from '../callout-badge';
...
import {
  getFrameCalloutTitleInputStyle as getCalloutTitleInputStyle,
  ...
  return (
    <>
      ...
      {props.showInteractionChrome !== false
        ? createPortal(renderCalloutInteractionHandles(props), controlsPortalTarget)
        : null}
      {props.showInteractionChrome !== false
        ? renderCalloutFloatingToolbar({
...
export function CalloutBody(props: CalloutBodyProps) {
  return createPortal(renderCalloutPortalContent(props), props.portalTarget);
}
