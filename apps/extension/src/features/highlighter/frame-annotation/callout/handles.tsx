  ...
  return (
    <>
      <button
        ...
        title={label}
        style={getWaypointHandleStyle(props)}
      ...
      {renderWaypointAngle(props)}
    </>
  );
}

function getWaypointHandleStyle(props: CalloutInteractionHandleProps) {
  return mergeThemeScopedStyle(props.portalTheme, {
    ...
    opacity: props.isHandleVisible ? 1 : 0,
    pointerEvents: props.isHandleVisible ? 'auto' : 'none',
    boxShadow: '0 1px 5px color-mix(in srgb, var(--sniptale-color-shadow-strong) 24%, transparent)',
    transform: props.isPolylineWaypoint ? 'rotate(45deg)' : undefined,
  } as CSSProperties);
}

function renderWaypointAngle(props: CalloutInteractionHandleProps) {
  if (
