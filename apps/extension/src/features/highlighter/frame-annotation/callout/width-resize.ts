...
import { registerPointerDragSession } from '../interaction/pointer-drag-session';
import type {
  ...
  React.useEffect(() => {
    ...
    const handleMove = (event: PointerEvent) => {
      ...
      const nextDraft = getDraft({
        active,
        clientX: (
          callout.coordinateSpace ?? identityFrameAnnotationCoordinateSpace
...
export function useCalloutWidthResize(args: CalloutWidthResizeArgs) {
  ...
  const handlePointerDown = (side: ResizeSide, event: CalloutWidthResizeStartEvent) => {
    ...
    const point = coordinateSpace.clientPointToLogical({ x: event.clientX, y: 0 });
    event.preventDefault();
    event.stopPropagation();
    ...
    state.activeRef.current = {
      fixedEdgeX: side === 'left' ? rect.x + rect.width : rect.x,
      fixedCenterY: rect.y + rect.height / 2,
