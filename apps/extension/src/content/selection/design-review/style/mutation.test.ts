...
it('applies, undoes, and redoes declarations inside an open shadow root', () => {
  const host = appendVisible(document.createElement('section'));
  const shadowRoot = host.attachShadow({ mode: 'open' });
  const target = appendVisible(document.createElement('div'), shadowRoot);
  const applied = applyPageStyleMutation({
    declarations: [{ property: 'color', value: 'red' }],
    target,
  });
...
it('fails closed when a custom element reenters direct declaration writes', () => {
  ...
  const host = appendVisible(document.createElement('section'));
  const shadowRoot = host.attachShadow({ mode: 'open' });
  const target = appendVisible(
    document.createElement(elementName) as ReentrantStyleElement,
    shadowRoot
  );
