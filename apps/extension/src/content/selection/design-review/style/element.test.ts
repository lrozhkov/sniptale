...
describe('page-style Element target eligibility', () => {
  ...
  it('accepts rendered page-owned shadow descendants for exact DOM review', () => {
    ...
    expect(isPageStyleMutationElement(openTarget)).toBe(true);
    expect(isPageStyleMutationElement(closedTarget)).toBe(true);
  });
  ...
  it('rejects content-owned UI while accepting page lookalikes', () => {
    ...
    lookalike.className = 'sniptale-design-review-control';
    lookalike.dataset['ui'] = 'content.designReview.control';
