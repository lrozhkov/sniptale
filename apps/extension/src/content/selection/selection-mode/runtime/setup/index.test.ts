...
import { applySelectionModeMutableRefSetters, createSelectionModeMutableRefGetters } from '.';
...
function updateRefsThroughSetters(refs: SelectionModeMutableRefs) {
...
it('exposes live getter-backed mutable refs for the runtime state', () => {
...
it('applies setter-backed mutable refs to the runtime owner state', () => {
  const state = createMutableRefState();
  ...
  const { cleanupEventListeners, cleanupScrollListeners, dom, hoveredElement } =
    updateRefsThroughSetters(refs);
