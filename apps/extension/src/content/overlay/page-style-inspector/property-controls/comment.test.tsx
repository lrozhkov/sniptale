...
function createActions(): PageStyleInspectorActions['comment'] {
  return {
    commit: vi.fn(() => true),
...
function createState(
  overrides: Partial<PageStyleInspectorViewState['comment']> = {}
): PageStyleInspectorViewState['comment'] {
  return {
    commitFailed: false,
    draft: 'Review this',
    marker: 3,
...
it('uses AA secondary text pairs for marker, placeholder, and hint copy in both themes', () => {
  ...
  expect(hint?.className).toContain('text-[var(--sniptale-color-text-secondary)]');
...
it('ships the comment field and marker copy in both supported locales', () => {
  expect(translate('content.pageStyleInspector.commentLabel', 'ru')).toBe('Комментарий к элементу');
  expect(translate('content.pageStyleInspector.commentLabel', 'en')).toBe('Element comment');
  expect(translate('content.pageStyleInspector.commentCommitFailed', 'en')).toContain(
    'draft is still here'
  );
});
