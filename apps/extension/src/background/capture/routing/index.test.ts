...
function resetCaptureRouterMocks() {
  vi.clearAllMocks();
  ...
  mockRoutesAsHandled(handleExecuteSaveMock);
  mockRoutesAsHandled(
    handleExportStartHarMock,
    handleExportStopHarMock,
    handleExportCaptureFullPageMock
...
function verifiesSaveRoutes(args: ReturnType<typeof createRouteArgs>) {
  expect(
    routeCaptureMessage({
      ...args,
      message: {
...
function verifiesExportRoutes(args: ReturnType<typeof createRouteArgs>) {
  expect(
    routeCaptureMessage({
      ...args,
      message: { type: MessageType.EXPORT_START_HAR, sessionId: 'har-1' },
    })
  ).toBe(true);
  expect(handleExportStartHarMock).toHaveBeenCalledWith(
    { type: MessageType.EXPORT_START_HAR, sessionId: 'har-1' },
    42,
    args.sendResponse,
    undefined
  );
...
function verifiesSaveAndExportRoutes() {
  ...
  verifiesSaveRoutes(args);
  verifiesExportRoutes(args);
}

describe('capture-router', () => {
  beforeEach(resetCaptureRouterMocks);
  it('routes save and export actions to the action handlers', verifiesSaveAndExportRoutes);
});
