...
const { createScenarioProjectRecordMock, translateMock } = vi.hoisted(() => ({
...
it('reuses the active scenario project without creating a new one', async () => {
  ...
  expect(createScenarioProjectRecordMock).not.toHaveBeenCalled();
...
it('creates and selects an auto-named project when the session has no active project', async () => {
  ...
  expect(createScenarioProjectRecordMock).toHaveBeenCalledWith(
