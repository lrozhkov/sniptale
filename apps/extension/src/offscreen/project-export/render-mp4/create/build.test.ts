...
it('collects only audio fallback notes when AAC is unavailable', async () => {
  const project = createProject() as never;
  const settings = createMp4Settings();
  const pipeline = await createMp4Pipeline(project, settings);

  expect(ensureMp4ExportSupportMock).toHaveBeenCalledWith(true);
  expect(renderOfflineAudioMixMock).toHaveBeenCalledWith(project, settings, undefined);
  expect(getSupportedMp4VideoEncoderMock).toHaveBeenCalledWith(settings, VideoMp4Codec.AVC);
