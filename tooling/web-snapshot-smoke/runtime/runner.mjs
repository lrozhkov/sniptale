...
import { createSmokeCaseVerifier } from '../assertions/case-verification.mjs';
...
async function collectFiles(root, directory = root) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) files.push(...(await collectFiles(root, entryPath)));
    else if (entry.isFile()) files.push(relative(root, entryPath).replaceAll('\\', '/'));
    else throw new Error(`Unsupported Web Snapshot candidate entry: ${entryPath}`);
  }
  return files.sort();
}

export async function createFixtureCandidateReceipt({ distRoot, selectedCaseNames }) {
  if (!Array.isArray(selectedCaseNames) || selectedCaseNames.length === 0) {
    throw new Error('Web Snapshot fixture receipt requires at least one selected case');
  }
  if (
    selectedCaseNames.some((name) => typeof name !== 'string' || name.length === 0) ||
    new Set(selectedCaseNames).size !== selectedCaseNames.length
  ) {
    throw new Error('Web Snapshot fixture receipt requires unique non-empty case names');
  }
  const files = await collectFiles(distRoot);
  if (files.length === 0) throw new Error('Web Snapshot fixture receipt requires a non-empty dist');
  const hash = createHash('sha256');
  for (const file of files) {
    hash.update(file);
    hash.update('\0');
    hash.update(await readFile(join(distRoot, file)));
    hash.update('\0');
  }
  return {
    schemaVersion: 1,
    mode: 'fixtures',
    candidateDigest: hash.digest('hex'),
    candidateFileCount: files.length,
    selectedCases: [...selectedCaseNames],
  };
}

export async function withFixtureRuntimeWorkspace({
  root,
  resultRoot,
  ...
  const userDataDir = await mkdtemp(join(temporaryRoot, 'sniptale-web-snapshot-smoke-'));
  let unpackedDir = null;
  try {
    unpackedDir = await mkdtemp(join(temporaryRoot, 'sniptale-web-snapshot-extension-'));
    await cp(join(root, 'dist'), unpackedDir, { recursive: true });
    return await execute({ resultRoot, unpackedDir, userDataDir });
  } finally {
    await rm(userDataDir, { force: true, recursive: true });
    if (unpackedDir) await rm(unpackedDir, { force: true, recursive: true });
  }
}

export async function runFixtureSmoke() {
  const fixtureBytes = await readFile(
    join(ROOT, 'tooling/web-snapshot-smoke/fixtures/complex.html')
  );
      ...
      execute: async ({ unpackedDir, userDataDir }) => {
        ...
        try {
          ...
          const report = {
            ...
            receipt: await createFixtureCandidateReceipt({
              distRoot: join(ROOT, 'dist'),
              selectedCaseNames: selectedCases.map(({ name }) => name),
            }),
          };
          ...
          for (const item of passed) {
            if (item.downloadProof) {
              requireCase(
                Boolean(item.downloadProof.filename),
                item,
                'download did not publish a filename'
              );
              continue;
            }
            ...
            requireCase(
              item.defaultStaticVisible,
              item,
              'static document is not the default viewer surface'
            );
            ...
            if (item.name === 'fixture') {
              ...
              requireCase(
                Boolean(
                  item.sourceInfo.inlineMaskImage && item.sourceInfo.inlineMaskImage !== 'none'
                ) &&
                  Boolean(
                    item.staticInfo.inlineMaskImage && item.staticInfo.inlineMaskImage !== 'none'
                  ),
                item,
              ...
              requireCase(
                Boolean(
                  item.sourceInfo.escapedMaskImage && item.sourceInfo.escapedMaskImage !== 'none'
                ) &&
                  Boolean(
                    item.staticInfo.escapedMaskImage && item.staticInfo.escapedMaskImage !== 'none'
                  ),
                item,
            ...
            if (popupUi) {
              ...
              requireCase(
                !progressRegressed,
                item,
                'popup replayed an already completed progress step'
              );
              requireCase(
