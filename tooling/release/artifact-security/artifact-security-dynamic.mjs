...
function collectFunctionAliasScopeEnds(sourceFile) {
...
function verifyAliasedFunctionConstructors(relativePath, snapshot) {
  const { sourceFile, text } = snapshot;
  const scopeEnds = collectFunctionAliasScopeEnds(sourceFile);
...
export function verifyDynamicCodeUsage(relativePath, textOrSnapshot) {
  const snapshot =
    typeof textOrSnapshot === 'string'
      ? getArtifactSourceSnapshot({ relativePath, text: textOrSnapshot })
      : textOrSnapshot;
  const { text } = snapshot;
  verifyDirectFunctionConstructors(relativePath, text);
  verifyAliasedFunctionConstructors(relativePath, snapshot);
}
