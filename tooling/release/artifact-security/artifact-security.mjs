...
import { getArtifactSourceSnapshot } from '../artifact-source/artifact-source-snapshot.mjs';
...
import { assertReleaseLegalPayload } from '../policy/oss-release-policy.mjs';
import {
...
const JAVASCRIPT_FILE_PATTERN = /\.(?:js|mjs)$/u;
...
const HASHED_SECRET_REDACTION_BUNDLE_PATTERN = /^assets\/secret-redaction-[a-z0-9_-]{8}\.js$/iu;
const FORBIDDEN_TEXT_PATTERNS = [
  ...
  { message: 'dynamic code execution', pattern: /(?:\beval\s*\(|\(\s*eval\s*\)\s*\()/u },
...
function collectPolicyNames(policy, key) {
  ...
  if (!Array.isArray(entries)) {
    throw new Error(`Release artifact manifest policy field ${key} must be an array.`);
  }
  const names = entries.map((entry) => entry?.name);
  if (names.some((name) => typeof name !== 'string') || new Set(names).size !== names.length) {
    throw new Error(`Release artifact manifest policy field ${key} has invalid names.`);
  }
...
function assertManifestPermissionPolicy(manifest, policy) {
  ...
  assertSetEquals(
    ...
    collectPolicyNames(policy, 'optionalPermissions'),
...
export function getForbiddenReleaseArtifactPathReason(relativePath) {
  ...
  for (const { message, pattern } of FORBIDDEN_FILE_PATTERNS) {
    if (
      ...
      !(
        message === 'secret-like artifact' &&
        (HASHED_UI_TOKENS_BUNDLE_PATTERN.test(normalizedPath) ||
          HASHED_SECRET_REDACTION_BUNDLE_PATTERN.test(normalizedPath))
      )
    ) {
      return message;
    }
...
function verifyTextContents(relativePath, contents) {
  ...
  if (JAVASCRIPT_FILE_PATTERN.test(relativePath)) {
    verifyDynamicCodeUsage(relativePath, getArtifactSourceSnapshot({ relativePath, text }));
  }
}
