...
function readPolicy(cwd) {
  ...
  if (
    ...
    typeof value.owners.artifactSeal !== 'string' ||
...
function collectRootFiles(cwd, root, excludedNames, output) {
  ...
  if (!fs.existsSync(absoluteRoot)) throw new Error(`Unit proof input root is missing: ${root}`);
...
export function parseFullUnitProof(value) {
  if (
    value?.schemaVersion !== 1 ||
...
function readProof(proofPath) {
  try {
    return parseFullUnitProof(JSON.parse(readRegularFile(proofPath, 'utf8')));
  } catch (error) {
