...
import { collectCoverageViolations, resolveCoverageTargetFiles } from './check.mjs';
import {
...
async function collectSettingsPageCoverageFeedback(root: string) {
  return withCwd(root, async () => {
    const module = await importFresh<typeof import('./check.mjs')>(
      '../proof/coverage/test-coverage/check.mjs'
    );
    ...
    expect(
      resolveCoverageTargetFiles({
        changedWorkspaceFiles: [
          ...
          'tooling/qa/proof/coverage/test-coverage/check.mjs',
        ],
      })
