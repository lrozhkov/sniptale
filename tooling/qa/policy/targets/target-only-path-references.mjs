...
function isTestPath(path) {
  return /(?:\.test\.|\.spec\.|\.test-support\.|\/test-support\/|(?:^|\/)test-support\.[cm]?[jt]sx?$)/u.test(
    path
  );
}
...
function classifyReference(path, policy) {
  ...
  if (
    path === 'tooling/qa/guards/product-contracts/build-layout/extension-build-layout-policy.mjs' ||
    path ===
      'tooling/qa/guards/product-contracts/extension-build/verify-extension-build-layout.mjs' ||
    path === 'tooling/qa/guards/product-contracts/package-boundaries/check.mjs'
  ) {
