...
it('allows non-domain exhaustiveness and boundary casts', () => {
  const root = createTempRoot('domain-fixture-realism-non-domain-');
  const good = writeFile(
    root,
    'apps/extension/src/workflows/media-hub-backup/restore/index.test.ts',
    [
      'it("checks an exhaustive branch", () => value as never);',
      'it("adapts a local helper", () => value as unknown as LocalFixture);',
      '',
    ].join('\n')
  );
...
it('finds multiline domain assertions and ignores nearby invalid prose', () => {
  const root = createTempRoot('domain-fixture-realism-multiline-');
  const bad = writeFile(
    root,
    'apps/extension/src/video-editor/project/state.test.ts',
    [
      '// malformed input belongs to a different test below',
      'it("restores a video project", () => ({ id: "project-1" } as',
      '  VideoProject));',
      '',
    ].join('\n')
  );

  expect(collectDomainFixtureRealismViolations([bad])).toEqual([
    expect.objectContaining({ line: 2, rule: 'domain-fixture-realism' }),
  ]);
});
...
it('does not cancel a new fixture identity against a removed cast', async () => {
  const root = createTempRoot('domain-fixture-realism-identity-swap-');
  initGitRepo(root);
  const file = 'apps/extension/src/video-editor/project/state.test.ts';
  writeFile(root, file, 'it("loads a video project", () => value as VideoProject);\n');
  runGit(root, 'add', '.');
  runGit(root, 'commit', '-m', 'baseline');
  writeFile(root, file, 'it("loads a scenario project", () => value as ScenarioProject);\n');
  ...
  expect(result.violations).toEqual([expect.objectContaining({ rule: 'domain-fixture-realism' })]);
});
