...
import { extensionBuildLayoutErrors } from '../build-layout/extension-build-layout-policy.mjs';
...
} from '../build-layout/extension-build-layout-policy.mjs';
...
export function collectExtensionBuildLayoutViolations({ rootDir = repoRoot } = {}) {
  ...
  const buildFiles = collectFiles(rootDir, 'apps/extension/build').filter(
    (file) => file !== EXTENSION_BUILD_LAYOUT_PATH || policy.configPaths.includes(file)
  );
...
export function runExtensionBuildLayoutCheck(options = {}) {
  return { violations: collectExtensionBuildLayoutViolations(options) };
}
