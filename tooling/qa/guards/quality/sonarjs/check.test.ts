...
import { importSonarjsVerifier, writeSonarjsTsconfig } from './test-support';

const verifierScript = path.join(process.cwd(), 'tooling/qa/guards/quality/sonarjs/check.mjs');
...
it('passes a clean production fixture', async () => {
  ...
  writeFile(
    ...
    [
      ...
      'export function call(width: number, height: number) { return draw(width, height); }',
      "export const sameType = '1' === '1';",
      'const values = [1, 2, 3];',
      'values.reverse();',
      'export const first = values[0];',
      'declare function getPromise(): Promise<number>;',
      'export async function promiseTry() { try { return await getPromise(); } catch { return 0; } }',
      'export const total = values.reduce((sum, item) => sum + item, 0);',
      '',
    ].join('\n')
  );
...
it('retains every type-aware curated hardfail rule against minimal fixtures', async () => {
  ...
  expect(result.violations.map((violation) => violation.rule).sort()).toEqual(
    verifier.SONARJS_TYPE_AWARE_RULE_IDS.toSorted()
  );
});

it('ignores app-generated/support scopes while retaining package vendor and generated production', async () => {
  ...
  const packageVendor = writeFile(
    root,
    'packages/platform/src/vendor/example.ts',
    'export const value = 1;\n'
  );
  const packageGenerated = writeFile(
    root,
  ...
  expect(seenFiles).toEqual([productionFile, packageGenerated, packageVendor].toSorted());
});

it('skips an empty focused production scope with a valid empty baseline', async () => {
  const root = createTempRoot('verify-sonarjs-empty-');
  writeSonarjsTsconfig(root);
  const verifier = await importSonarjsVerifier(root);

  const result = await withCwd(root, () =>
    verifier.runSonarjsCheck({ files: ['package.json'], lintFiles: async () => [] })
  );

  expect(result).toEqual({ files: [], skipped: true, violations: [] });
});

it('keeps fatal parser errors blocking on the real residual execution path', async () => {
  const root = createTempRoot('verify-sonarjs-parser-');
  writeSonarjsTsconfig(root);
  writeFile(root, 'apps/extension/src/broken.ts', 'export const broken = ;\n');
  const verifier = await importSonarjsVerifier(root);

  const result = await withCwd(root, () => verifier.runSonarjsCheck({ scope: 'repo-wide' }));

  expect(result.violations).toEqual([
    expect.objectContaining({ file: 'apps/extension/src/broken.ts', rule: 'sonarjs-eslint' }),
  ]);
});
...
it('keeps report-only CLI findings non-blocking', () => {
  ...
  writeSonarjsTsconfig(root);
  writeFile(root, 'apps/extension/src/bad.ts', "export const badCompare = '1' === 1;\n");
  ...
  expect(result.stderr).toContain('sonarjs/different-types-comparison');
});
