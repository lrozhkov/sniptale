...
it('derives current controller methods and follows raw aliases and destructuring', () => {
  const root = createTempRoot();
  const file = writeFile(
    root,
    'apps/extension/src/editor/inspector/sidebar-controller/builders.ts',
    [
      ...
      'export function buildArgs(controller: ImageEditorController) {',
      '  const alias = controller;',
      '  const { redo } = alias;',
      '  return { redo, direct: alias.redo };',
      '}',
    ].join('\n')
  );

  expect(collectDetachedControllerMethodViolations([file])).toEqual([
    expect.objectContaining({ rule: 'detached-controller-method' }),
    expect.objectContaining({ rule: 'detached-controller-method' }),
  ]);
});

it('allows data properties and binding to the receiver but rejects binding elsewhere', () => {
  const root = createTempRoot();
  const file = writeFile(
    root,
    'apps/extension/src/editor/inspector/sidebar-controller/builders.ts',
    [
      ...
      '  const safe = controller.setActiveTool.bind(controller);',
      '  const unsafe = controller.setActiveTool.bind(null);',
      '  return { canvas, safe, unsafe };',
      '}',
    ].join('\n')
  );
