...
it('keeps oxlint scoped to JS-like files and skips empty scopes', async () => {
  ...
  ).toEqual(['tooling/qa/guards/quality/verify-oxlint.mjs']);
...
it('uses the single checked-in Oxlint policy authority', async () => {
  ...
  module.runOxlint({
    files: ['tooling/qa/guards/quality/verify-oxlint.mjs'],
...
it('binds strict security mode and scheduler CPU tokens to the same Oxlint process', async () => {
  ...
  expect(() =>
    module.runOxlint({ files: ['tooling/qa/guards/quality/verify-oxlint.mjs'], threads: 0 })
...
it('runs the TS6 contract enum guard over the same canonical target closure', async () => {
  ...
  const contractEnumCollector = vi.fn(() => [
    {
      rule: 'contract-enum',
      file: 'tooling/qa/guards/quality/verify-oxlint.mjs',
      line: 1,
      column: 1,
  ...
  expect(contractEnumCollector).toHaveBeenCalledWith([
    'tooling/qa/guards/quality/verify-oxlint.mjs',
  ]);
...
it('expands oxlint directories for release scans', async () => {
  ...
  expect(calls[0]?.[1]).toEqual(
    expect.arrayContaining(['tooling/qa/guards/quality/verify-oxlint.mjs'])
  );
});
