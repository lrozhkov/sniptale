...
import { collectRootScatterViolations, runRootScatterCheck } from './check.mjs';
import { RETIRED_ROOTS } from './config.mjs';
...
function allowsSanctionedThinFacadeRoots() {
  const root = createTempRoot();
  const file = writeFile(
    root,
    'apps/extension/src/background/runtime.ts',
    "export { startRuntime } from './runtime/start-runtime';\n"
  );
...
function flagsRetiredRootFiles() {
  ...
  expect(
    collectRootScatterViolations([scriptsFile, testsFile, harnessFile], {
      root,
    })
  ).toEqual([
    expect.objectContaining({
...
function reportsRepoWideRetiredSharedRootStyleWrappers() {
  ...
  expect(runRootScatterCheck({ root, scope: 'repo-wide' }).violations).toEqual([
    expect.objectContaining({
      file: 'apps/extension/src/popup/bootstrap.test.ts',
      rule: 'root-owner-test-smell',
    }),
...
function preservesExplicitEmptyFocusedScope() {
  const root = createTempRoot();
  writeFile(root, 'apps/extension/src/background/helpers.ts');

  expect(runRootScatterCheck({ files: [], root })).toEqual({ files: [], violations: [] });
}

function reportsNestedFilesForEveryConfiguredRetiredRoot() {
  const root = createTempRoot();
  const retiredRoots = RETIRED_ROOTS;
  for (const retiredRoot of retiredRoots) {
    writeFile(root, `${retiredRoot}/nested/phantom.ts`);
  }

  const result = runRootScatterCheck({ root, scope: 'repo-wide' });

  expect(result.violations).toHaveLength(retiredRoots.length);
  expect(result.violations.map(({ file }) => file).sort()).toEqual(
    retiredRoots.map((retiredRoot) => `${retiredRoot}/nested/phantom.ts`).sort()
  );
  expect(result.violations.every(({ rule }) => rule === 'retired-root-owner-smell')).toBe(true);
}
...
describe('collectRootScatterViolations', () => {
  ...
  it('allows sanctioned thin facade roots', allowsSanctionedThinFacadeRoots);
  ...
  it('preserves an explicit empty focused scope', preservesExplicitEmptyFocusedScope);
  it(
    'reports nested files for every configured retired root',
    reportsNestedFilesForEveryConfiguredRetiredRoot
  );
});
