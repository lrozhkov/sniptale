...
function createSourceFile(relativePath, sourceText, version = 'current') {
...
export function collectNamedExports(
  relativePath,
  sourceText = readText(relativePath),
  version = 'current'
) {
  const sourceFile = createSourceFile(relativePath, sourceText, version);
...
function collectManualMocks(testFile, sourceText = readText(testFile), version = 'current') {
  const sourceFile = createSourceFile(testFile, sourceText, version);
  const mocks = [];
...
function collectMissingExports(mock, modulePath, moduleSource, version = 'current') {
  const exports = collectNamedExports(modulePath, moduleSource, version);
...
function collectNetNewMissingExports({ currentMocks, mockIndex, mockedModule, testFile }) {
  ...
  const previousMocks = collectManualMocks(testFile, previousTestSource, 'HEAD');
  const previousMock = findPreviousMock(currentMocks, previousMocks, mockIndex);
  const previousMissing = collectMissingExports(
    previousMock,
    mockedModule,
    previousModuleSource,
    'HEAD'
  );
