...
async function runCheck(root: string, targetFiles: string[]) {
  return withCwd(root, async () => {
    const module = await importFresh<typeof import('./check.mjs')>('./check.mjs', import.meta.url);
...
it('fails object mocks that miss named exports from a changed module', async () => {
  ...
  expect(result.violations[0]?.message).toContain('missing exports: readExample.');
});

it('ignores erased type-only exports when a changed mock covers every runtime export', async () => {
...
it('checks value re-exports without requiring colocated type re-exports', async () => {
  const root = createTempRoot('manual-mock-parity-mixed-reexports-');
  writeFile(root, 'src/shared/example/runtime.ts', 'export const runtimeValue = true;\n');
  writeFile(root, 'src/shared/example/types.ts', 'export type Example = { id: string };\n');
  writeFile(
    root,
    'src/shared/example/index.ts',
    [
      "export { runtimeValue, type MissingRuntimeShape } from './runtime';",
      "export type { Example } from './types';",
      '',
    ].join('\n')
  );
  writeFile(root, 'src/feature/example.test.ts', "vi.mock('../shared/example', () => ({}));\n");

  const result = await runCheck(root, ['src/shared/example/index.ts']);

  expect(result.violations).toEqual([
    expect.objectContaining({
      message: expect.stringContaining('runtimeValue'),
    }),
  ]);
  expect(result.violations[0]?.message).not.toContain('MissingRuntimeShape');
  expect(result.violations[0]?.message).not.toContain('Example');
});

it('does not let a type-only star re-export disable runtime export checking', async () => {
  const root = createTempRoot('manual-mock-parity-type-star-');
  writeFile(root, 'src/shared/example/types.ts', 'export type Example = { id: string };\n');
  writeFile(
    root,
    'src/shared/example/index.ts',
...
it('reports only a net-new deficit when an export is added beside existing debt', async () => {
  ...
  expect(result.violations).toEqual([
    expect.objectContaining({
      message: expect.stringContaining('addedReader'),
    }),
  ]);
});
