...
const BROWSER_ADAPTER_OWNERS_POLICY_PATH =
  'tooling/qa/policy/browser-adapters/browser-adapters-owners.mjs';
...
export function collectBrowserAdapterViolations(
  files,
  { astGrepReceipt = null, root = repoRoot } = {}
) {
  ...
  const result = astGrepReceipt
    ? projectAstGrepReceipt(astGrepReceipt, ['browser-adapters'])
    : runAstGrepCheck({
        files: relevantFiles,
...
export function runBrowserAdapterCheck({ astGrepReceipt = null, files = [], root = null } = {}) {
  ...
  return {
    ...
    violations: [
      ...
      ...collectBrowserAdapterViolations(targetFiles, { astGrepReceipt, root: resolvedRoot }),
    ],
  };
}
