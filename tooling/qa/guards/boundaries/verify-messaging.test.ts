...
function verifiesNamespaceDynamicAndReExportViolations() {
  const root = createTempRoot();
  const namespace = writeFile(
    root,
    'apps/extension/src/content/logic/namespace.ts',
    [
      "import * as runtimeMessaging from '../../platform/runtime-messaging';",
      'runtimeMessaging.sendRuntimeMessage({ type: "PING" });',
      '',
    ].join('\n')
  );
  const dynamic = writeFile(
    root,
    'apps/extension/src/content/logic/dynamic.ts',
    [
      'export async function send() {',
      "  const { sendTabMessage } = await import('../../platform/runtime-messaging');",
      '  return sendTabMessage(1, { type: "PING" });',
      '}',
      '',
    ].join('\n')
  );
  const reExport = writeFile(
    root,
    'apps/extension/src/content/logic/re-export.ts',
    "export { sendRuntimeMessage } from '../../platform/runtime-messaging';\n"
  );
  const star = writeFile(
    root,
    'apps/extension/src/content/logic/star.ts',
    "export * from '../../platform/runtime-messaging/default-transport';\n"
  );

  expect(
    collectDefaultRuntimeMessagingImportReport([namespace, dynamic, reExport, star], { root })
  ).toMatchObject({
    added: expect.arrayContaining([
      expect.objectContaining({ importName: 'sendRuntimeMessage' }),
      expect.objectContaining({ importName: 'sendTabMessage' }),
      expect.objectContaining({ importName: '*' }),
    ]),
  });
}

function verifiesTypeOnlyReExportsAreAllowed() {
  const root = createTempRoot();
  const file = writeFile(
    root,
    'apps/extension/src/content/logic/types.ts',
    "export type { RuntimeMessagingTransport } from '../../platform/runtime-messaging';\n"
  );
  expect(collectDefaultRuntimeMessagingImportReport([file], { root }).violations).toEqual([]);
}

function verifiesDirectTabsMessagingViolation() {
  const root = createTempRoot();
  const file = writeFile(
    root,
    'apps/extension/src/content/logic/example.ts',
    'export const send = () => chrome.tabs.sendMessage(1, { type: "PING" });\n'
  );
  expect(collectMessagingViolations([file], { root })).toEqual([
    expect.objectContaining({ rule: 'messaging-direct-send' }),
  ]);
...
function verifiesDefaultRuntimeMessagingImportBaselineRemoval() {
  ...
  expect(report.violations).toEqual([
    expect.objectContaining({
      file: 'tooling/qa/policy/messaging/messaging.mjs',
...
describe('collectMessagingViolations', () => {
  ...
  it(
    'flags namespace, dynamic and re-export access to default transports',
    verifiesNamespaceDynamicAndReExportViolations
  );
  it('allows type-only re-exports', verifiesTypeOnlyReExportsAreAllowed);
  it(
    'flags direct tabs messaging outside the transport owner',
    verifiesDirectTabsMessagingViolation
  );
  it(
