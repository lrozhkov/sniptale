...
it('blocks multiline schema assertions and double-cast escapes in current contract owners', async () => {
  ...
  const file = writeFile(
    root,
    'apps/extension/src/contracts/messaging/contracts/schema.ts',
    [
      'declare const z: { object(value: object): unknown };',
      'declare namespace z { type ZodType<T> = { parse(value: unknown): T } }',
      'type RuntimeMessage = { type: string };',
      'const schema = z.object({}) as',
      '  z.ZodType<RuntimeMessage>;',
      'export function escape(candidate: unknown) {',
  ...
  await expect(collectViolations(root, [file])).resolves.toEqual(
    expect.arrayContaining([
      expect.objectContaining({ rule: 'boundary-schema-type-escape' }),
      expect.objectContaining({ rule: 'boundary-schema-double-cast' }),
    ])
  );
});

it('allows a contract escape only in the branch that validates the same boundary value', async () => {
  const root = createTempRoot('qa-boundary-casts-validated-');
  const file = writeFile(
    root,
    'packages/runtime-contracts/src/messaging/parsers/native.ts',
    [
      'type RuntimeMessage = { type: string };',
      'declare function isRuntimeMessage(value: unknown): boolean;',
      'export function parse(candidate: unknown) {',
      '  return isRuntimeMessage(candidate)',
      '    ? candidate as unknown as RuntimeMessage',
      '    : null;',
      '}',
      '',
    ].join('\n')
