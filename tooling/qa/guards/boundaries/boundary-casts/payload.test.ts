...
import { collectBoundaryCastViolations } from './check.mjs';
...
it('flags unknown response and payload casts', () => {
  ...
  const file = writeFile(
    ...
    [
      ...
      'declare const runtime: { subscribe(callback: (result: unknown, errorData: unknown) => void): void };',
      'runtime.subscribe((result: unknown, errorData: unknown) => {',
