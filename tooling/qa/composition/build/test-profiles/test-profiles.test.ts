...
import { createUnitTestPlan } from '../../../proof/unit/unit-test-plan.mjs';
...
it('keeps a deleted modal facade chain with an untested successor on transitive proof', async () => {
  ...
  const result = await withCwd(root, async () => {
    const scopeModule = await importFresh<typeof import('../scope/scope.mjs')>(
      '../scope/scope.mjs',
      import.meta.url
    );
    const preflightModule = await importFresh<
      typeof import('../../preflight/guardrail-preflight-scope.mjs')
    >('../../preflight/guardrail-preflight-scope.mjs', import.meta.url);
