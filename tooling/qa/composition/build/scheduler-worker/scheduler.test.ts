...
import { resolveQaReleaseResourceProfile } from '../../../runtime/scheduling/resource-profile.mjs';
import { collectScheduledBuildStepResults, runBuildLaneWorker } from './scheduler.mjs';
import { resolveBuildTestScope } from '../scope/scope.mjs';
...
it('runs a real 198-input related scope in the saturated phase', async () => {
  ...
  const workerRunner = vi.fn(async ({ lane }: { lane: string }) =>
    lane === 'tests'
      ? {
          testSteps: [{ ...step('Unit tests'), detail: testScope.detail }],
        }
      : laneValue(lane)
  );
  const saturatedProfile = {
...
it('executes a build test lane in a real worker without starting Vite', async () => {
  ...
  expect(value.testSteps.map(({ status }) => status)).toEqual(['skipped']);
});
