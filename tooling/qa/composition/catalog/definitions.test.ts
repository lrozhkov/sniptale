...
import { VERIFY_ALL_VIOLATION_STEPS } from '../repository/full-verification/violation-steps.mjs';
import {
  FOCUSED_CODE_VIOLATION_STEPS,
  FOCUSED_CONTEXTUAL_VIOLATION_STEPS,
} from '../checkpoint/focused/code-steps.mjs';
...
it('covers executable focused and release guardrail definitions without a shadow registry', () => {
  ...
  expect(labels(lanes['release-guardrail']).filter((label) => label !== 'Logging policy')).toEqual(
    VERIFY_ALL_VIOLATION_STEPS.map(([label]) => label)
  );
  expect(labels(lanes['focused-guardrail']).filter((label) => label !== 'Logging policy')).toEqual(
