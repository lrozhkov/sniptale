...
async function collectReport(root, input) {
  return withCwd(root, async () => {
    const module = await importFresh<typeof import('./check.mjs')>(
      '../composition/preflight/guardrail-preflight-report/check.mjs'
    );
...
it('reports path-sensitive registry hints for missing moved paths referenced in quality-gate data', async () => {
  ...
  writeFile(
    root,
    'tooling/qa/guards/quality/example.rules.mjs',
...
it('discovers exact registry authorities and ignores ordinary or unrelated config files', async () => {
  const root = createTempRoot('guardrail-registry-authorities-');
  ...
  writeFile(
    root,
    'tooling/configs/qa/unrelated.data.json',
    `${JSON.stringify({ path: movedPath })}\n`
  );
  writeFile(
    root,
    'tooling/qa/guards/quality/ordinary.mjs',
    `export const path = '${movedPath}';\n`
  );
...
it('classifies every tooling QA owner family as quality-gate tooling', async () => {
  const root = createTempRoot('guardrail-tooling-clusters-');
  const targetFiles = [
    'tooling/qa/guards/quality/example.mjs',
    'tooling/qa/composition/checkpoint/example.mjs',
    'tooling/qa/runtime/process/example.mjs',
    'tooling/qa/wrappers/checkpoint.mjs',
  ];
  for (const file of targetFiles) writeFile(root, file, 'export {};\n');

  const report = await collectReport(root, { targetFiles, codeFiles: targetFiles });

  expect(report.clusters).toEqual(['tooling:quality-gates=4']);
  expect(report.hints).toContain(
    'wrapper/tooling seam changed: include wrapper-contract and runner tests'
  );
});
