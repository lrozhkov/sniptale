...
function expectHarnessChangeInvalidatesFullDiff(args: {
  ...
  context: ReturnType<typeof import('../scope/qa-scope.mjs').createScopedQaContext>;
  harnessChangedContext: ReturnType<typeof import('../scope/qa-scope.mjs').createScopedQaContext>;
}) {
...
it('validates checkpoint state freshness against the current diff fingerprint', async () => {
  ...
  await withCwd(root, async () => {
    ...
    const qaScope = await importFresh<typeof import('../scope/qa-scope.mjs')>(
      '../scope/qa-scope.mjs',
      import.meta.url
    );
    const checkpointHelpers = await importFresh<
      typeof import('./verify-checkpoint.state.helpers.mjs')
    >('./verify-checkpoint.state.helpers.mjs', import.meta.url);
