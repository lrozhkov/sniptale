...
it('resolves canonical CI scope from the repository snapshot rather than the current diff', () => {
  ...
  const harnessFile = 'tooling/qa/composition/repository/full-verification/scope.mjs';
...
it('keeps deleted files in the diff scope without routing them to source readers', async () => {
  ...
  const result = await withCwd(root, async () => {
    const module = await importFresh<typeof import('./scope.mjs')>('./scope.mjs', import.meta.url);
