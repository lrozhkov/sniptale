...
it('keeps verify-all violation steps in the documented focused-to-broad order', async () => {
  const { VERIFY_ALL_VIOLATION_STEPS } =
    await import('../repository/full-verification/violation-steps.mjs');
...
it('keeps focused guardrail runners on explicit existing code files', () => {
  const executionSource = fs.readFileSync(
    'tooling/qa/composition/checkpoint/focused/execution.mjs',
    'utf8'
  );
  const codeStepsSource = fs.readFileSync(
    'tooling/qa/composition/checkpoint/focused/code-steps.mjs',
    'utf8'
  );
...
it('keeps qa:checkpoint self-contained with advisory and a blocking-wrapper lock', () => {
  ...
  const cliSource = fs.readFileSync('tooling/qa/wrappers/checkpoint/checkpoint-cli.mjs', 'utf8');
  const advisoryHelper = fs.readFileSync(
    'tooling/qa/composition/advisory/advisory-report.helpers.mjs',
    'utf8'
  );
...
it('keeps qa:closeout as checkpoint plus qa:build commit handoff', () => {
  ...
  const handoffSource = fs.readFileSync(
    'tooling/qa/wrappers/closeout/closeout-build-handoff.mjs',
    'utf8'
  );
  const packageJson = JSON.parse(fs.readFileSync('package.json', 'utf8')) as {
    scripts: Record<string, string>;
