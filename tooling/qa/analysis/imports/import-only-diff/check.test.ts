...
it('detects import-only diffs after a staged rename', async () => {
  ...
  const result = await withCwd(root, async () => {
    const module = await loadModule(root);
    return {
      importOnly: module.isImportOnlyDiffFile('src/new-owner/example.test.ts'),
      renameSource: module.collectRenameSourceByTarget().get('src/new-owner/example.test.ts'),
    };
  });

  expect(result).toEqual({
    importOnly: true,
    renameSource: 'src/old-owner/example.test.ts',
  });
});

it('pairs deleted tracked sources with edited untracked rename targets without mutating the index', async () => {
  const root = createTempRoot('import-only-diff-untracked-rename-');
  initGitRepo(root);
  writeFile(
    root,
    'src/old-owner/example.test.ts',
    [
      ...
      ...Array.from(
        { length: 10 },
        (_, index) => `expect(value).toBeTruthy(); // stable assertion ${index}`
      ),
      '',
    ].join('\n')
  );
  ...
  fs.mkdirSync(path.join(root, 'src/new-owner'), { recursive: true });
  fs.renameSync(
    path.join(root, 'src/old-owner/example.test.ts'),
    path.join(root, 'src/new-owner/example.test.ts')
  );
  writeFile(
    root,
    'src/new-owner/example.test.ts',
    [
      "import { value } from '../shared/value/index';",
      '',
      ...Array.from(
        { length: 10 },
        (_, index) => `expect(value).toBeTruthy(); // stable assertion ${index}`
