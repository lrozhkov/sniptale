...
import { toRelativePath } from '../repository/shared-paths.mjs';
import { getSourceSnapshot } from './source-snapshot.mjs';
...
export function scanRepoScopedTypeScriptFiles(
  ...
  for (const filePath of files) {
    ...
    const sourceSnapshot = getSourceSnapshot({ filePath });

    visitFile({
      filePath,
      normalizedPath,
      relativePath,
      sourceFile: sourceSnapshot.sourceFile,
      sourceSnapshot,
    });
  }
}
