...
import { GITLEAKS_NORMALIZATION_SCHEMA } from './gitleaks-baseline.mjs';

function writeBaseline(baselinePath: string, findings: unknown[] = []) {
  fs.writeFileSync(
    baselinePath,
    JSON.stringify({
      schemaVersion: 1,
...
it('rejects a baseline fingerprint that disagrees with its finding metadata', async () => {
  ...
  ).toThrow('fingerprint does not match its complete tuple');
});
