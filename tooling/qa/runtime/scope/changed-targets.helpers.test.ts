...
function verifiesRenameTargets() {
  ...
  expect(targets.addedFiles).toEqual(['src/new-owner.ts']);
  expect(targets.deletedFiles).toEqual(['src/old-owner.ts']);
  expect(targets.changedFiles).toEqual(['src/new-owner.ts', 'src/old-owner.ts']);
  expect([...(targets.changedLineMap.get('src/new-owner.ts') ?? [])]).toEqual([]);
}

function verifiesStableStagingIdentity() {
  const beforeStage = createChangedFileTargets({
    deletedFiles: ['src/old-owner.ts'],
    untrackedFiles: ['src/new-owner.ts'],
  });
  const afterStage = createChangedFileTargets({
    renamedFiles: ['src/new-owner.ts'],
    renamedSources: ['src/old-owner.ts'],
  });

  expect(afterStage.addedFiles).toEqual(beforeStage.addedFiles);
  expect(afterStage.deletedFiles).toEqual(beforeStage.deletedFiles);
  expect(afterStage.changedFiles).toEqual(beforeStage.changedFiles);
}
...
describe('createChangedFileTargets', () => {
  ...
  it('normalizes staged renames to candidate delete-and-add targets', verifiesRenameTargets);
  it('keeps target identity stable across staging rename detection', verifiesStableStagingIdentity);
});
