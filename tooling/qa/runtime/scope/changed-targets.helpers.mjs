...
} from '../../analysis/git/git-fallback.mjs';
...
export function createChangedFileTargets({
  ...
  // A candidate must retain the same identity before and after `git add -A`.
  // Normalize Git's staged rename classification to the delete+add shape that
  // the workspace necessarily exposes while the destination is still untracked.
  const addedFileSet = new Set([...addedFiles, ...untrackedFileSet, ...renamed]);
  const deletedFileSet = new Set([...deletedFiles, ...sources]);
  const changedFiles = [
