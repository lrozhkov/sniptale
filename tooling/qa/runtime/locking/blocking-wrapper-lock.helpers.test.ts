...
it('mentions canonical local and CI wrappers in the shared blocking-wrapper lock guidance', async () => {
  ...
  await withCwd(root, async () => {
    ...
    const lock = module.acquireBlockingWrapperLock('ci:release');
    expect(() => module.acquireBlockingWrapperLock('qa:build')).toThrow(
      /qa:checkpoint, qa:closeout, qa:build, qa:release-harness, ci:proof, or ci:release/u
    );
    lock.release();
  });
});
