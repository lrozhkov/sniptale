...
Changed-line readability remains a local diff control and is satisfied in Fast proof by the separate repository-wide readability control. Structural risk uses a repository scan with an exact current-debt baseline, and UI automation seams use their repository-wide adapter. Mandatory file controls report their repository population; an unexplained zero-file population cannot pass.

- `ci:proof` runs the complete repository-wide deterministic control set, read-only formatting, SonarJS, full product coverage, all product and harness unit tests, the Fast audit profile, and exactly one fresh `npm run build:release` result named `Production build`. That build creates no ZIP or reusable build receipt. The proof claims the Fast Gate but not release readiness.
