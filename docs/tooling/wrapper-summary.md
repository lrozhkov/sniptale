...
- `ci:release` requires the exact Fast proof and does not rerun its unit suite or reused `Production build` control. It separately adds release-only SonarJS, Build and Release archive plus the full release audit with blocking jscpd/CodeQL/coverage/legal and supply-chain controls. If an exact reusable Fast proof is unavailable, the same wrapper first executes the Fast prerequisite on the current machine and then runs the release-only delta. A fresh prerequisite supplies the one live product `npm audit` result to the release audit; reused Fast proof instead runs npm vulnerability and signature checks once in the release audit. No release path runs either owner twice.
