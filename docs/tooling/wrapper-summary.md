...
The first step emitted by each scheduled lane records queue time, lane wall time, CPU/memory reservation, and the selected resource profile. Build unit-test detail also records independent selection and execution decisions, the related input count, and the execution reason; for example, `selection=related-transitive; execution=saturated-exclusive; related-inputs=198; reason=related-input-threshold; budget=12cpu/14969MiB`. Dead-export steps record whether the persistent import/export index was rebuilt, incrementally updated, or reused. These values are diagnostic state; harness/checkpoint/build fingerprints remain the proof authority.
