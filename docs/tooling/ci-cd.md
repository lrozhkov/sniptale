...
The scheduled TTL sweeper runs only after the current workflow cleanup and uses the immutable controller image for the trusted commit. It deletes ports and offline runner registrations only when their complete managed identity is expired; a merely detached port or offline but unexpired JIT runner is not sufficient. This prevents sweeper/provision races and makes every rented network resource visible in cleanup proof.
...
`release.yml` is manual deployment only and never provisions Selectel. It accepts an existing signed `v<package version>` tag and a successful Release provenance Gate run for the same commit. Release proof is admissible only when its candidate and trusted control digests are equal to the control digest already present at that commit on `main`; a PR candidate-controls proof cannot be promoted directly. By default the named run must be the latest successful provenance run for that commit. An explicit older-run bypass requires an operator note, which remains in the GitHub run summary.
