...
Before publication, dispatch Continuous Deployment with `diagnostic: true`, the signed tag, release notes, and the exact successful `main` provenance run. Require the `Continuous Deployment diagnostic Gate` to pass and inspect its preserved deployment artifact. Diagnostic mode may run from a temporary fix branch, performs the real read-only admission and preparation without canonical QA, and cannot enter `release-publisher` or mutate GitHub Releases. Publish from `main` with the same inputs only after that diagnostic is green.
