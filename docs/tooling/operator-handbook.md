...
| Local PR bypass proof | `npm run ci:proof -- --pr <number>` | Run from a clean checkout exactly at `origin/main`; accepts only owner-authored PRs, uses separate tracked-only candidate/trusted exports, posts hashes after a final authority check, and leaves native bypass merge manual. |
...
Release publication accepts only the machine-owned release principal as both the original and triggering GitHub actor. It checks the live repository immutable-release setting and exact active no-bypass `v*` tag update/deletion ruleset before it runs canonical QA, then rechecks the exact verified tag object, peeled workflow commit, ruleset, and immutable-release setting immediately before creating the draft. The exact draft release ID, asset set, and GitHub-computed digests are verified before publication makes them immutable, and the published immutable release is verified again afterward. The live-setting check first tries the workflow token; if that token cannot read repository Administration state, configure `RELEASE_POLICY_TOKEN` as a fine-grained read-only Administration token. A missing or unreadable live setting fails closed before release publication. The QA image resolves Debian and Playwright system dependencies from the immutable Debian snapshot recorded in `toolchain.lock.json`, then installs Playwright from its separate integrity-locked npm closure and digest-verified Chromium/headless-shell/FFmpeg archives.

Pull-request required-check authority uses `pull_request_target`: GitHub loads the workflow and trusted CI control plane from the base commit, checks out the exact public candidate SHA separately without credentials, builds the locked image only from the trusted base, and executes the candidate workspace inside that container. Result jobs consume only artifacts admitted by the trusted collector; candidate code never receives the workflow token or OIDC capability.
