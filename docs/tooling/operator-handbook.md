...
Checkpoint formatting is always sequential and finishes before any verification lane starts. After that barrier, wrappers select a bounded WSL-visible profile. The default caps are 8 CPU tokens, 12 GiB estimated resident memory, and 4 concurrent Vitest workers. An explicit measured Vitest override may raise that worker cap to 6 when the CPU budget permits. Build is always exclusive. Operator overrides are positive integers: `SNIPTALE_QA_CPU_TOKENS`, `SNIPTALE_QA_MEMORY_MIB`, and `SNIPTALE_QA_VITEST_MAX_WORKERS`; the memory budget has a 4096 MiB minimum because the scheduler retains the real 3–4 GiB heavy-lane reservations instead of relabelling them as smaller work. Overrides are clamped to resources visible inside WSL and never mean `auto`.
