...
Checkpoint formatting is always sequential and finishes before any verification lane starts. After that barrier, wrappers select a bounded WSL-visible profile. The default checkpoint and related/direct build-test caps are 8 CPU tokens, 12 GiB estimated resident memory, and 4 concurrent Vitest workers. An explicit measured Vitest override may raise that normal worker cap to 6 when the CPU budget permits. A build/closeout full-suite fallback and `qa:release` are intentionally different: after their non-test prerequisite phase finishes, repo-wide Vitest runs as a second exclusive phase with up to 12 WSL-visible CPU tokens, 12 workers, and all visible WSL memory except 1 GiB. Build remains exclusive after tests. Operator overrides are positive integers: `SNIPTALE_QA_CPU_TOKENS`, `SNIPTALE_QA_MEMORY_MIB`, and `SNIPTALE_QA_VITEST_MAX_WORKERS`; they can reduce full-suite saturation when Windows is busy. The release profile requires at least 2 CPU tokens, while its memory budget has a 6144 MiB minimum because the scheduler retains the real heavy-lane reservations instead of relabelling them as smaller work. Overrides are clamped to resources visible inside WSL and never mean `auto`.
