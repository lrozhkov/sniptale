...
GitHub Actions treats pull-request source and dependency hooks as untrusted. The required gate is defined by the default-branch `pull_request_target` workflow: it checks out the trusted base control plane and exact candidate head SHA separately without persisted credentials, builds the QA image only from trusted base inputs, and executes the candidate only as a tokenless container workspace through trusted wrapper and artifact owners. Candidate code never receives the workflow token or OIDC authority. The isolated `pr-gate-authority` job alone receives `checks: write` and publishes the required GitHub Actions `pr-gate` Check Run against the exact candidate head SHA after all three canonical lanes succeed; its native base-associated job name is not a required context. External Actions are pinned to full commit SHAs. GitHub Releases must have repository-enforced immutability enabled before publication; the exact mutable draft receives and verifies the complete asset set before publication locks the tag and assets.
