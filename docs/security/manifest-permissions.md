...
| Web-copy redirect guard | `declarativeNetRequest` session rules | background Web Snapshot asset fetch owner | Extension-initiated tabless GET only; block HTTP and syntactically private/local hops before dispatch, fail before fetch if rule installation fails. |
