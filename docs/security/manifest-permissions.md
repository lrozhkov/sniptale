...
| Offscreen media and voice input | `offscreen`, `USER_MEDIA` | shared offscreen-document lifecycle, video runtime, and voice-input runtime | Video recording and voice-recognition sessions share one offscreen media lease. The Settings test consumer has a 30-second session deadline; authorized content consumers remain active until their owner explicitly stops or disconnects. Microphone access is requested only by an explicit trusted action in visible extension-owned UI, either an extension page or an authorized top-level content overlay; signed commands, exact sender and single-owner Port policy, freshness, and runtime capability checks remain aligned. |
