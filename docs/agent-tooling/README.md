...
The command copies `AGENTS.md`, `DESIGN.md`, and `.agents/**` into the repository root. Those destinations are ignored by Git. Existing files are left untouched when they differ from this kit; use `npm run agents:install -- --force` only when replacing local changes is intentional.
