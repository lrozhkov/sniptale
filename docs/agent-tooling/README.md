...
Create a shareable derived archive with:

```bash
npm run agents:pack
```

The deterministic archive is written to ignored `artifacts/agent-tooling.zip`. The text in this directory remains the source of truth; do not commit the archive.
