...
2. Verify that the scope is one coherent ownership result and that the manifest covers the complete selected root/seam rather than a representative sample. Treat the owner/change-reason cluster as the unit, classify each candidate as `Split`, `Consolidate`, or `Keep`, record the current independent change reason for every proposed owner, and use only known accepted adjacent changes to reject an immediately brittle form. Hypothetical future changes must not create current owners, contracts, or configuration. When supplied maintenance evidence includes overlapping forwarding-edge candidates, verify every forwarding-only single-production-consumer edge instead of trusting a path-partition `Consolidate: 0` summary.
...
List findings first, highest severity first, with evidence, violated invariant, concrete reachable failure/risk, and minimal correction class. Compare before/after navigation transitions, facade/proxy/pass-through layers, public surface, state authorities, effects/recovery, cohesion, and current independent change reasons. Treat a mechanical line-count split or a set of neighboring files that still share one broad state/effect contract as a distributed god-object, not a completed topology improvement; also reject consolidation that minimizes file count by erasing explicit architectural boundaries. Do not reject or report a bounded current shape merely because a hypothetical future feature might prefer another abstraction unless the user explicitly requested future-topology planning. Then state manifest completeness, topology posture, move-mechanic/proof-map posture, residual assumptions, and one decision: `Approve`, `Approve with comments`, or `Request changes`.
