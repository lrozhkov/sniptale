...
Build an evidence map from source, active documentation, package scripts, wrappers, QA policy data, and supplied output. QA validation manifests live under `tooling/configs/qa/validation-manifest.json`; the legacy core location is not authoritative. Audit structural/advisory/topology-fragmentation policy drift across source, catalogs, lanes, manifests, artifact schemas, and active guidance, but do not launch a repository-wide structural detector or collect model-token/token-hotspot inventories. A separately supplied manual `qa:structural-audit` artifact may provide report-only `Split`/`Consolidate`/`Keep` owner-cluster maintenance evidence; it is never whole-repository acceptance proof.
